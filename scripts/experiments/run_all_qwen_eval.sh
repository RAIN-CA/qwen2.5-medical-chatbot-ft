...
run_eval () {
  ...
  OUT_DIR="outputs/experiments/${family}/${size}/${run_type}"
  ...
  if [[ "$run_type" == "base" ]]; then
    python -m src.eval.run_inference_on_test \
      ...
      --test_file "${TEST_FILE}" \
      --output_file "${OUT_DIR}/predictions/all_domains.jsonl" \
  ...
  python -m src.eval.evaluate_predictions \
    --pred_file "${OUT_DIR}/predictions/all_domains.jsonl" \
    --report_file "${OUT_DIR}/reports/all_domains_report.json"
}
